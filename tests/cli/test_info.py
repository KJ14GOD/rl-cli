...
def test_info_summarizes_single_run() -> None:
    with runner.isolated_filesystem():
        init_result = runner.invoke(app, ["init", "bossfight"])
        assert init_result.exit_code == 0

        run_dir = Path("bossfight/runs/tiny_info_001")
        _write_fake_run(run_dir=run_dir)

        result = runner.invoke(app, ["info", "tiny_info_001"])

        assert result.exit_code == 0
        assert "RLCLI Run Overview" in result.stdout
        assert "RLCLI Run Performance" in result.stdout
        assert "RLCLI Run Artifacts" in result.stdout
        assert "RLCLI Config Summary" in result.stdout
        assert "tiny_info_001" in result.stdout
        assert "CartPole-v1" in result.stdout
        assert "manual_eval_001.json" in result.stdout
        assert "videos/manual_video_001/manifest.json" in result.stdout
        assert "algo.learning_rate" not in result.stdout
...
def _write_fake_run(*, run_dir: Path) -> None:
    ...
    metadata = {
        ...
        "source_config_path": "bossfight/configs/tiny_info.yaml",
        "git_commit": "deadbeef",
        "total_timesteps": 256,
        ...
        "last_video_at": "2026-04-13T10:00:07Z",
        "last_video_manifest": "videos/manual_video_001/manifest.json",
    }
    (run_dir / "metadata.json").write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
    (run_dir / "metrics.jsonl").write_text(
        json.dumps({"step": 64, "rollout/ep_rew_mean": 32.0}) + "\n"
        + json.dumps({"step": 256, "rollout/ep_rew_mean": 48.5}) + "\n",
        encoding="utf-8",
    )
    ...
    video_dir = run_dir / "videos" / "manual_video_001"
    video_dir.mkdir()
    (video_dir / "manifest.json").write_text(
        json.dumps(
            {
                "kind": "standalone_video",
                "summary": {"mean_reward": 50.0, "mean_episode_length": 108.0},
            },
            indent=2,
        )
        + "\n",
